...
public class Configs {
	...
	public void setup() {
		try {
			...
			builder.configure(params.properties().setListDelimiterHandler(new DefaultListDelimiterHandler(',')).setFileName(path.getAbsolutePath()));
	...
	public String getJarPath() {
		try {
			File file = new File(Configs.class.getProtectionDomain().getCodeSource().getLocation().toURI().normalize().getPath());
	...
	public Path getImageLocation(String img) {
		...
		return isValidPath(location) ? Paths.get(location).normalize().toAbsolutePath() : getImageLocation("not_found.png");
	}
